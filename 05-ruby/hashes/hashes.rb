...
users["Jorge"] = {:twitter => 'edwardawkward', :favorite_numbers => [23, 69, 666, 12]}
...
p users["Anil"][:favorite_numbers].select { |n| n.even? } # alternatively: .select(&:even?)
...
# Fancy 1: users.map { |name, info| info[:favorite_numbers] }.inject { |memo, nums| memo & nums }
# Fancy 2: users.map { |name, info| info[:favorite_numbers] }.inject(&:&)
...
all_nums = users["Jonathan"][:favorite_numbers] +
            ...
            users["Jorge"][:favorite_numbers]
p all_nums.sort.uniq
# Fancy: users.map { |name, info| info[:favorite_numbers] }.flatten.sort.uniq
